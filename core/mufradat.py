...
# Узбекский добавлен 30.08.2026 (в группах есть братья-узбеки) - КИРИЛЛИЦЕЙ,
# решение пользователя: студенты выросли и учились здесь, кириллица им
# привычнее латиницы, плюс визуально ложится в один ряд с ru/ky.
SUPPORTED_LANGUAGES = {"ru": "Русский", "ky": "Кыргызча", "uz": "Ўзбекча"}
DEFAULT_LANGUAGE = "ru"
...
# невозможно.
#
# Порог ОДИН на три вещи сразу - это и делает его чувствительным:
#   - слово уходит на RECHECK_AFTER_DAYS-дневный отдых (is_mastered);
#   - забытое слово возвращается из "Мои слова" в норму
#     (record_answer -> remove_starred_by_progress_key);
#   - ЗНАМЕНАТЕЛЬ оценки знания "Общий вес" (word_stimulus_credit =
#     streak / MASTERY_STREAK, дальше score10).
#
# 30.08.2026: 5 -> 3, ПО МЕТОДИКЕ. Порог сначала опустили до 3, затем
# вернули 5 (чтобы не ломать шкалу "Общего веса"), и это была ошибка -
# порог подгонялся под удобство метрики. Пользователь поймал: "важно
# высчитать оптимальный порог, а не подгонять порог под корректность веса".
#
# Замер на реальном pick_question_word (40 ответов/день, 120 дней) -
# сколько слов реально ДОХОДИТ до отдыха:
#     пул   35 слов: порог 3 -> 35/35 (за 3 дня)    порог 5 -> 35/35 (6 дней)
#     пул  200 слов: порог 3 -> 200/200 (16 дней)   порог 5 -> 200/200 (30 дней)
#     пул 1000 слов: порог 3 -> 1000/1000 (77 дней) порог 5 -> 203/1000 (112 дней)
#
# На пуле активного студента (15+ страниц, 1000+ слов) при пороге 5 за
# четыре месяца до отдыха доходит лишь пятая часть слов - механика отдыха
# фактически мертва. Причина: слово там показывают раз в ~20 дней, а вес
# 1/(1+streak) делает почти закреплённое слово ещё и втрое реже нового.
#
...
#
# Охват непоказанных слов этот порог НЕ решает - им занимается волна
# (COVERAGE_WAVE_SHARE ниже), при ней слов "не показано ни разу" ноль и при
# 3, и при 5. Это две независимые задачи, их больше не путать.
#
# Цена решения, принята осознанно: знаменатель "Общего веса" - тот же
# MASTERY_STREAK, поэтому оценка разом выросла у всех примерно на 63% без
# новых ответов (замер по проду: 12981 запись, средний вклад слова
# 0.245 -> 0.400, "закреплено" 0 -> 775 слов). Так и должно быть - критерий
# закреплённости уточнён, и выученное слово обязано давать в оценке 100%,
# иначе вес 10 недостижим в принципе (решение пользователя 30.08.2026).
MASTERY_STREAK = 3
