...
# ── Напоминание подключить ЛС с ботом (пн и чт, 08:00) ────────────────────────
...
# предупреждения о пропусках. Групповой чат, где бот уже состоит: публикуем
# ссылку-приглашение только для тех, кто ещё не подключился; группа, где все
# уже подключены, ничего не получает. С 12.08.2026 дополнительно - личное
# сообщение устазу группы с именами таких студентов, чтобы он мог дожать
# лично (не только групповой пост, который легко пропустить) - всем
# админам группы, включая супер-админов, если они сами устаз этой группы
# (решение пользователя 12.08.2026: где-то супер-админ - единственный
# реальный устаз, исключать его было бы лишать группу единственной помощи).

async def dm_connect_reminder():
    """Пн и чт (12.08.2026, было ежедневно - "замыливается", решение
    пользователя). Кроме группового поста - теперь ещё личное сообщение
    устазу группы с именами его студентов, которые не нажали Start, чтобы
    он лично мог дожать (тот же повод - активный студент Салих, G-12,
    сдаёт почти каждый день, но бот не может писать ему в личку)."""
    ...
    for group in get_all_groups():
        ...
        try:
            ...
            for admin_phone in get_group_admins(group["id"]):
                if not admin_phone:
                    continue
                # Без исключения по SUPER_ADMIN_IDS (12.08.2026, решение
                # пользователя): кто в этой группе устаз - тому и уходит,
                # независимо от того, супер-админ он или нет, т.к. где-то
                # супер-админ и есть единственный реальный устаз, помочь
                # больше некому (не тот же принцип, что в
                # invite_missing_ustaz_to_scaling - там речь про приглашение
                # в отдельный канал, не про помощь конкретному студенту).
                await send_message(
                    admin_phone,
...
async def profile_survey_intro():
    """Раз в день - разовый личный вопрос "откуда ты и сколько лет", теперь
    через 2 дня после перевода в постоянную группу (user_groups.joined_date),
    задаётся по очереди в два шага (13.08.2026, редизайн 17.08.2026 - было
    "30 дней после регистрации", решение пользователя: между регистрацией и
    реальным началом мог быть большой разрыв). Ответы приходят обычным
    текстом в личку и ловятся в handlers.py по survey_stage - здесь только
    отправляем первый вопрос и переводим анкету в состояние ожидания.

    Выключено по умолчанию (bot_settings["profile_survey_enabled"] != "1").
    17.08.2026 - пользователь решил запустить разово всем подходящим сразу
    (без батчинга по 5/день, как думали раньше) - но пока только на
    мужском боте; на женском set_setting не выставлять, пока не скажет
    отдельно (bot_settings не общий между ботами, так что это просто вопрос
    того, где выставлен флаг)."""
    if get_setting("profile_survey_enabled") != "1":
        return
    for row in get_users_due_for_survey():
        try:
...
async def profile_survey_nudge():
    """Раз в день - повтор вопроса анкеты тем, кто застрял на
    'asked_location'/'asked_age' ≥7 дней без ответа (17.08.2026). Повторяет
    ТОТ ЖЕ вопрос, на котором остановились, не с начала. Заодно частично
    лечит случаи устаревшего dm_ok=1 (сообщение реально не дошло в первый
    раз) - следующая попытка через неделю может пройти."""
    if get_setting("profile_survey_enabled") != "1":
        return
    for row in get_users_due_for_survey_nudge():
        try:
            group = get_learning_group(row["phone"])
            glang = get_group_lang(group) if group else "ru"
            key = {
    ...
    while True:
        try:
            ...
            if h == 7 and m == 0:
                ...
                if wd in (0, 3):  # пн и чт - 2 раза в неделю (12.08.2026, было ежедневно)
