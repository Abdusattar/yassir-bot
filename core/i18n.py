...
TR = {
    ...
    "prep_failed_group": {
        "ru": "📿 {name} — за 14 дней подготовительного периода сдал(а) только {days} дн. из нужных 5, поэтому переведён(а) в Тадаббур. Вернуться можно будет, когда будет готовность сдавать регулярно 🤲",
    ...
    "prep_graduate_announce_old": {
        "ru": "🎓 {name} успешно прошёл(а) подготовительный период и переходит в группу «{title}».\nПусть Аллах облегчит его(её) путь в изучении Корана — дуа за брата/сестру! 🤲",
    ...
    "prep_graduate_announce_new": {
        "ru": "🎉 Ассаляму алейкум! Устаз, дорогие братья — встречайте {name}: успешно прошёл(а) подготовительный период и присоединяется к нашей группе.\nПожелаем ему(ей) терпения и берекета на этом пути! 🤲",
