...
_DEFAULT_KNOWLEDGE = [
    "ПРОЕКТ ЯССИР: система заучивания Корана с дословным переводом слов (муфрадат). "
    "Цель — чтобы люди ЖИЛИ с Кораном: понимали каждое слово, чувствовали его красоту и смысл. "
    "Студент не просто читает Коран — он его ПОНИМАЕТ.",

    "ГЛАВНЫЙ ПРИНЦИП МЕТОДИКИ: сначала муфрадат (значения всех слов отрывка) → "
    "потом хифз (заучивание наизусть). Без знания слов заучивание НЕ начинается.",

    "МЕТОД 40+40: 40 раз глядя в текст + 40 раз не глядя наизусть. "
    "Страница делится на 2 половины (~7-8 строк каждая). "
    "1 этап: каждую строку отдельно по 40+40. "
    "2 этап: соединить все строки половины по 40+40. "
    "3 этап: вся страница целиком по 40+40. "
    "40+40 — это МЕТОД, а не количество аятов. НЕ говори 'выучил 80 аятов'.",

    "ПЕРЕХОД В МЕТОДИКЕ: строка читается до начала СЛЕДУЮЩЕЙ строки. "
    "Новая строка начинается с её начала. Этот переход применяется на каждой строке.",

    "ЗАДАНИЯ (коды): m=Заучивание 40+40, r=Повторение/мурожаа, t=Слова/Перевод/муфрадат. "
    "Дополнительно: j=Таджвид, n=Нахв/Грамматика, h=Хадис (слово с переводом). "
    "Задания у каждой группы настраиваются устазом отдельно.",

    "ПРО-ГРУППЫ: строгий режим ежедневного хифза. "
    "Задания: минимум m + r + t (и дополнительные по настройке). "
    "Правило: 14 пропущенных дней за текущий месяц → бот автоматически переводит в Тадаббур. "
    "Предупреждение при 7+ пропусках в месяце.",

    "ОБЫЧНЫЕ ГРУППЫ: гибкий режим. Те же задания что в ПРО. "
    ...
    "При ≤3 пропусках за 30 дней → бот предлагает устазу перевести студента в ПРО.",

    "ТАДАББУР — НЕ учебная группа. Это пространство для всех кто любит Коран. "
    "Здесь передаётся красота и смысл Корана: аяты с тафсиром, наставления устазов, "
    "отчёты ПРО-групп (для вдохновения), ссылки на онлайн-уроки, объявления проекта. "
    "Открыт для всех — учащихся и ещё не начавших. "
    "Студенты попадают сюда при превышении лимита пропусков — временно, до восстановления активности.",

    "БАЛЛЫ: 1 балл за каждое выполненное задание в день. "
    "+5 бонусных баллов за 7 дней подряд без единого пропуска (серия). "
    "+5 баллов за присутствие на онлайн-уроке. "
    "Рейтинг внутри группы. /mystats — своя статистика. /rating — топ группы. "
    "Лучший за неделю и месяц получает публичную похвалу.",

    "РАСПИСАНИЕ БОТА: 07:00 утренняя мотивация (кто не сдал), "
    "18:00 личные напоминания, 20:00 вечерний отчёт группы + сводка в Тадаббур, "
    "20:30 предупреждения о пропусках, 21:00 проверка переводов. "
    "Воскресенье 19:00 — итог недели, 20:30 — Ясир спрашивает устаза об улучшениях. "
    "1-е число 19:00 — итог месяца. 1 января 11:00 — годовой итог.",

    "ЯСИР (бот) ДЕЛАЕТ: принимает отчёты студентов, фиксирует выполненные задания, "
    "отвечает на вопросы об арабском языке/Коране/методике/таджвиде/нахве, "
    "проверяет правильность ответов по таджвиду/нахву/муфрадату, "
    "хвалит выполнивших всё, мотивирует, предупреждает об опасных пропусках.",

    "ЯСИР (бот) НЕ ДЕЛАЕТ: не даёт фетвы (вопросы халяль/харам/фикх → к устазу лично), "
    "не заменяет устаза в личном наставничестве, "
    "на вопросы о жизни/здоровье/семье — мягко направляет к устазу. "
    "ВАЖНО: автоматические переводы в Тадаббур бот ВЫПОЛНЯЕТ сам по правилам (14/30 дней подряд). "
    "Предложения об апгрейде в ПРО отправляются устазу — устаз сам решает.",

    "ТРЕБОВАНИЯ К СТУДЕНТАМ: ежедневно сдавать отчёт в группу, "
    "соблюдать порядок (сначала муфрадат → потом хифз), "
    "честно отражать реальное выполнение заданий, "
    "при уважительной причине написать устазу.",

    "ОБЯЗАННОСТИ УСТАЗА: студенты регистрируются сами — вступают в группу, бот просит имя, они пишут. "
    "Настраивает задания (/settasks m,r,t), задаёт тип группы (/settype pro|relaxed|tadabbur), "
    "задаёт язык (/setlang ru|ky), "
    "настраивает куда переводить неактивных (/setfallback chat_id), "
    "подтверждает предложения об апгрейде в ПРО, проводит онлайн-уроки.",

    "ЦЕПОЧКА ЗАПУСКА ГРУППЫ: "
    "1) /setgroup — зарегистрировать группу в боте. "
    "2) /settasks m,r,t — задать задания (m=заучивание, r=повторение, t=слова, j=таджвид, n=нахв, h=хадис). "
    "3) /setlang ky — язык интерфейса (ru/ky/uz/kk/ar). "
    "4) /settype pro|relaxed|tadabbur — тип группы. "
    "5) /admin +телефон — назначить устаза группы (у каждой группы свой). "
    "6) /setfallback -chatid — куда переводить неактивных студентов. "
    "Студент: вступает → бот: «напиши своё имя» → пишет имя → зарегистрирован. "
...
def seed_default_knowledge():
    """Заполняет базу знаний правилами проекта при первом запуске (если пуста)."""
    with db() as c:
        count = c.execute("SELECT COUNT(*) FROM yassir_knowledge").fetchone()[0]
        if count == 0:
            for text in _DEFAULT_KNOWLEDGE:
                c.execute("INSERT INTO yassir_knowledge(text) VALUES(?)", (text,))


# Словарь: фрагмент СТАРОГО текста → НОВЫЙ полный текст
_KNOWLEDGE_FIXES = {
    "14 дней пропусков за месяц": (
        "ПРО-ГРУППЫ: строгий режим ежедневного хифза. "
        "Задания: минимум m + r + t (и дополнительные по настройке). "
        "Правило: 14 пропущенных дней за текущий месяц → бот автоматически переводит в Тадаббур. "
        "Предупреждение при 7+ пропусках в месяце."
    ),
    "30 дней пропусков. При 30": (
        "ОБЫЧНЫЕ ГРУППЫ: гибкий режим. Те же задания что в ПРО. "
        "Правило: 30 дней подряд без единого отчёта → бот автоматически переводит в Тадаббур. "
        "Предупреждение при 15+ последовательных пропусках. "
        "При ≤3 пропусках за 30 дней → бот предлагает устазу перевести студента в ПРО."
    ),
    "не принимает решения о переводах студентов (только устаз)": (
        "ЯСИР (бот) НЕ ДЕЛАЕТ: не даёт фетвы (вопросы халяль/харам/фикх → к устазу лично), "
        "не заменяет устаза в личном наставничестве, "
        "на вопросы о жизни/здоровье/семье — мягко направляет к устазу. "
        "ВАЖНО: автоматические переводы в Тадаббур бот ВЫПОЛНЯЕТ сам по правилам (14/30 дней подряд). "
    ...
    "Воскресенье 19:00 — итог недели. 1-е число 19:00 — итог месяца.": (
        "РАСПИСАНИЕ БОТА: 07:00 утренняя мотивация (кто не сдал), "
        "18:00 личные напоминания, 20:00 вечерний отчёт группы + сводка в Тадаббур, "
        "20:30 предупреждения о пропусках, 21:00 проверка переводов. "
        "Воскресенье 19:00 — итог недели, 20:30 — Ясир спрашивает устаза об улучшениях. "
...
def migrate_knowledge_fixes():
    """Исправляет некорректные записи и добавляет недостающие знания (запускается при каждом старте)."""
    with db() as c:
        rows = c.execute("SELECT id, text FROM yassir_knowledge").fetchall()
        # Исправить устаревшие записи
        for row in rows:
            for old_fragment, new_text in _KNOWLEDGE_FIXES.items():
                if old_fragment in row["text"]:
                    c.execute("UPDATE yassir_knowledge SET text=? WHERE id=?", (new_text, row["id"]))
                    break
        # Добавить недостающие записи из _DEFAULT_KNOWLEDGE
        all_text = " ".join(r["text"] for r in rows)
        for entry in _DEFAULT_KNOWLEDGE:
            marker = entry[:40]
            if marker not in all_text:
                c.execute("INSERT INTO yassir_knowledge(text) VALUES(?)", (entry,))
