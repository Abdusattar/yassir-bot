...
# Учебный день кончается не в полночь, а в три ночи (20.09.2026, решение
# пользователя). Причина - жалоба «пропуск, которого не было»: кто садится за
# задания поздно вечером, перешагивает полночь, и сдача уезжала в следующий
# день, а прошедший оставался пустым и считался пропуском. Замер по сентябрю:
# 26 человек из 195, 37 дней; у одного брата сдачи идут в 23:48-02:58 каждый
# день - он терял дни просто потому, что учится ночью.
...
def get_date():
    ...
    До трёх ночи идёт ещё вчерашний день. Это ЕДИНСТВЕННАЯ точка отсчёта для
...
def get_calendar_date():
    """Настоящая дата по календарю - для того, что живёт не по учебному дню:
    публикация уроков по расписанию, сроки, отчёты «за такое-то число»."""
    return datetime.now(pytz.timezone(TZ)).date().isoformat()


def in_night_tail():
    """Сейчас тот самый хвост суток, когда идёт ещё вчерашний учебный день.
    Нужно, чтобы бот сказал об этом словами, а не ставил человека перед
    молчаливым «сегодня уже всё сдано»."""
    return get_date() != get_calendar_date()
...
def is_app_member(phone):
    ...
    Своим считается тот, кто реально числится в ЭТОЙ базе: студент учебной
    группы (включая подготовительную - её студенты сдают те же задания) или
    устаз любой группы.

    Супер-админ САМ ПО СЕБЕ права не даёт, и это осознанно. SUPER_ADMIN_IDS
    у мужского и женского ботов пересекаются (в .env.female исторически
    прописаны и мужские id), и по одной этой строчке мужской супер-админ
    заходил бы в женскую половину. Решение пользователя 10.09.2026: Умар
    устаз и он остаются только в мужской, Зейнеб и Асель - только в
    женской. Проверено по базам: у первых двух в женской нет ни одной
    записи, у вторых - в мужской, так что членства достаточно всем.

    На то, что супер-админ ВИДИТ внутри приложения (все группы в кабинете),
...
# Служебная группа (09.09.2026): рабочий чат устазов, где никто не учится.
# Появилась из-за "Группы устазат" - она стояла обычным relaxed, и бот вёл
# её как учебную: вход туда выкидывал устаза из его настоящей группы
# (правило "один студент - одна группа"), сдачи из YassirApp улетали в неё,
# а за пропуски там начислялись штрафы. В служебной группе бот только
# присутствует: не считает её учебной (get_learning_group отбирает лишь
# pro/relaxed/prep), не принимает сдачи, не кикает, не шлёт напоминания.
SERVICE_GROUP_TYPE = "staff"

# Типы, которые бот не ведёт как учебные. Тадаббур - своя жизнь (насыха,
# сводки), служебная - вообще никакой.
UNLED_GROUP_TYPES = ("tadabbur", SERVICE_GROUP_TYPE)


def is_service_group(group_type):
    """Служебная группа - бот в ней только присутствует (см. SERVICE_GROUP_TYPE)."""
    return (group_type or "relaxed") == SERVICE_GROUP_TYPE


def bot_leads_group(group_type):
    """Ведёт ли бот эту группу как учебную: напоминания, отчёты, счёт
    пропусков, переводы. Для тадаббура и служебных групп - нет."""
    return (group_type or "relaxed") not in UNLED_GROUP_TYPES
...
# Имя из букв: кириллица (вкл. кыргызские/казахские), латиница, арабский.
# Цифр нет намеренно - "40 40" и "1 этап" именем быть не должны.
_NOT_NAME_WORDS = {
    "ок", "окей", "ok", "okay", "да", "нет", "ага", "угу", "хорошо", "ладно", "понял", "поняла",
    "понятно", "спасибо", "спс", "рахмат", "рахмет", "раxмат", "ооба", "жок", "ха", "йўқ", "yes", "no",
    "ин", "иншаа", "иншаллах", "иншааллах", "аллах", "аллаха", "машаллах", "машааллах", "альхамдулиллях",
    "альхамдулиллах", "субханаллах", "аминь", "амин", "сейчас", "щас", "уже", "готово", "сдал", "сдала",
    "тест", "test", "хай", "hi", "hello", "привет", "салам", "салом",
}
# Слова, по которым фраза уже точно не имя, даже рядом с другими словами.
_NOT_NAME_PHRASE_WORDS = {"джазакаллаху", "джазака", "джазакуллаху", "барака", "баракаллаху", "хайран",
                          "шаа", "аллах", "аллаху", "спасибо", "рахмат", "фик"}
_PLAIN_NAME_RE = re.compile(
    r"^[A-Za-zА-Яа-яЁёӨөҮүҢңҘҙІіЎўҚқҒғҲҳЖжШш\u0600-\u06FF'\-.\s]+$"
)
...
def looks_like_plain_name(text, report_check=True):
    ...
    этого КАЖДАЯ регистрация тратила два вызова ИИ - извлечение имени и
    проверку, что это имя, - и когда у OpenRouter кончились кредиты,
    регистрация встала совсем: бот бесконечно повторял «Напиши только своё
    имя» (Салия и Зульфия, женский бот, 08.09).

    Правило намеренно строгое: ошибиться в сторону «не имя» безопасно -
    такой текст просто уйдёт к ИИ, как раньше. Отчёт («Заучивание 40+40»,
    «Повторил») именем не станет: слова заданий проверяются отдельно."""
    raw = (text or "").strip()
    if not (2 <= len(raw) <= 32):
        return False
    if not _PLAIN_NAME_RE.match(raw):
        return False
    if len(raw.split()) > 3:          # «Салия. М.» - да, фраза - нет
        return False
    if looks_like_greeting(raw):      # «Ассаляму алейкум» - не имя
        return False
    # Частые ответы в чате - не имя (17.09.2026: «ок», «Спасибо», «да»
    # проходили как имя; у записанных без имени вопрос висит до 7 дней).
    words = [w for w in re.split(r"[\s.\-]+", raw.lower()) if w]
    if words and all(w in _NOT_NAME_WORDS for w in words):
        return False
