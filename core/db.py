...
_DEFAULT_KNOWLEDGE = [
    ...
    "18:00 личные напоминания, 20:00 вечерний отчёт группы, "
    ...
    "ОБЯЗАННОСТИ УСТАЗА: добавляет студентов (/add Имя или /add Телефон Имя), "
    "настраивает задания (/settasks m,r,t), задаёт тип группы (/settype pro|relaxed|tadabbur), "
    "задаёт язык (/setlang ru|ky), добавляет знания для бота (/teach текст), "
...
def migrate_knowledge_fixes():
    """Исправляет некорректные записи в базе знаний (запускается при каждом старте)."""
    with db() as c:
