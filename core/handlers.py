...
async def _verify_and_reply(chat_id, text, group_title, phone, group_id, name, checks, glang="ru", message_id=None, student_id=None):
    try:
        system = (
            ...
            "(подтверждено Умар устазом 24.07.2026). Пример: студент написал "
            "«رِزًا — اسم-معرب-نكرة» без роли (не назвал فاعل/مفعول به и т.п.) — "
            "это ВЕРНО, НЕ пиши «не указан иъраб»!\n"
