...
async def answer_question(question, program_info, group_title, phone=None, group_id=None, student_name="", is_ustaz=False):
    ...
    system = (
        ...
        + role_note + "\n"
        "ГРАНИЦА ЗНАНИЙ: отвечай СТРОГО на основании информации о проекте и академического "
        "справочника ниже — это единственный источник фактов, которому ты доверяешь. "
        "Не используй собственные знания о Коране, хадисах, тафсире, фикхе или истории вне "
        "того, что написано ниже, и не придумывай источники, если их там нет. "
        "Если вопрос выходит за рамки этой информации (общие религиозные вопросы, фетвы, "
        "тафсир конкретных аятов, хадисов, спорные богословские темы) — не отвечай по существу "
        "и не гадай, а скажи что это нужно уточнить у устаза лично.\n\n"
        + _HUMAN_STYLE + "\n\n"
        "Кратко и по делу, без воды. Простой язык понятный студенту. "
        "Если в справочнике ниже есть аят/хадис по теме — можно кратко процитировать с "
        "источником, но не за пределами справочника. "
        "Если студент написал перевод/таджвид/грамматику — проверь и укажи ошибки кратко "
        "(это проверка его текста по правилам арабского языка, а не ответ на общий вопрос — "
        "здесь можно опираться на знание грамматики и таджвида). "
...
async def answer_if_relevant(question, program_info, group_title, phone=None, group_id=None, student_name="", is_ustaz=False):
    ...
    system = (
        ...
        "ГРАНИЦА ЗНАНИЙ: если решил отвечать — используй СТРОГО информацию о проекте и "
        "справочник ниже, не собственные знания о Коране/хадисах/фикхе вне их. Если вопрос "
        "относится к боту, но ответа в справочнике нет (общий религиозный вопрос, фетва, "
        "тафсир и т.п.) — не придумывай, а коротко ответь что нужно уточнить у устаза "
        "(это не IGNORE, это нормальный короткий ответ).\n\n"
        + _HUMAN_STYLE + "\n\n"
        + PROJECT_INFO + "\n\n"
...
async def _build_source_block(hadith: dict | None, ayah: dict | None, lang: str) -> str:
    ...
    if hadith:
        ...
        block += (
            ...
            "Ссылка: (" + hadith.get("label", hadith.get("collection", "")) + ")\n\n"
        )
    return block
...
async def group_motivation(missing_names, group_title, lang="ru",
    ...
    prompt = (
        ...
        "- После упоминания смысла хадиса сразу в скобках: (Сборник)\n"
        "- Если в переданном хадисе упоминается, что по этому поводу был ниспослан аят — "
        "НЕ цитируй этот аят дословно в кавычках и НЕ подписывай его ссылкой на сборник хадиса "
        "(это разные источники). Просто скажи своими словами, что был ниспослан аят на эту тему, без цитаты.\n"
...
async def group_motivation_base(lang: str, gtype: str,
                                hadith=None, ayah=None, model=None, max_tokens=1024,
                                neutral: bool = False) -> str | None:
    """Насыха без имён. Один вызов на (gtype, lang); имена добавляет планировщик.

    neutral=True — без гендерного обращения и без приветствия внутри текста.
    Используется только для общего кэша Тадаббур-насыхи (один LLM-вызов на
    мужской и женский боты сразу — каждый подставляет своё приветствие сам)."""
    source_block = ""
    if ayah:
        meaning = ayah.get("meaning_en") or await _get_ayah_translation(ayah, lang)
        source_block += (
    ...
    if neutral:
        addr_instr = (
            "Обращение к аудитории — БЕЗ указания пола: не пиши «братья», не пиши «сёстры», "
            "не пиши «братья и сёстры». Используй нейтральные формы («дорогие студенты» и "
            "т.п.) или обходись вовсе без прямого обращения к аудитории. "
            "Не пиши приветствие («Ассаляму алейкум» и т.п.) в начале текста — оно добавляется "
            "отдельно, перед этим текстом."
        )
    else:
        addr_instr = _GENDER_ADDR
