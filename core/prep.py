...
# Кому лично уходит алерт "студент застрял сутки+" (не отвечает / не вступил) -
    ...
    for group in prep_groups:
        ...
        if passive_names:
            ...
            if glang == "ky":
                ...
                # «Полных» и «все три задания» (13.09.2026): день засчитывается
                # только со всеми тремя (count_report_days_since), а «≥5 дней»
                # читалось как «пять дней с любым заданием».
                lines.append(f"⏳ Пока не сдавали — период короткий, нужно 5 полных дней (все три задания) для перехода:")
...
async def _alert_if_stuck(student_id, group_id, joined, name, reason, since_ts):
    """Разово уведомляет пользователя (не Умара), если студент застрял на
    каком-то шаге >=24 часов (не отвечает на вопрос / получил ссылку, но не
    вступил) - решение пользователя 24.07.2026. Разово - потому что сам шаг
    (вопрос/ссылка) и так повторяется студенту каждую проверку; здесь просто
    сигнал "обрати внимание", не нужно спамить одним и тем же каждые 12 часов."""
    if not since_ts or not _LATE_ALERT_ADMIN_ID:
        return
    try:
        hours = (datetime.utcnow() - datetime.strptime(since_ts, "%Y-%m-%d %H:%M:%S")).total_seconds() / 3600
    except ValueError:
        return
    if hours < 24:
        return
    ...
    with db() as c:
        already = c.execute(
            ...
            "AND category='prep_late_alert' AND subcategory=?", (student_id, group_id, key)
        ).fetchone()
    if already:
        return
    add_bonus(student_id, group_id, joined, 0, "prep_late_alert", subcategory=key)
    await send_message(_LATE_ALERT_ADMIN_ID,
        "⏰ " + name + " уже " + str(round(hours)) + " ч. — " + reason)
...
async def send_prep_onboarding_group_message(chat_id, name, glang, dm_ok, uid=None, note=""):
    ...
    from core.side import half_word
    if dm_ok:
