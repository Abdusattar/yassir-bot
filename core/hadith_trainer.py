...
* **Одно новое слово в день**, строго. Сверх нормы - только «Ещё заход»:
  повтор выученного.
...
* **Весь хадис на экране** (24.09.2026, решение пользователя) — под
  ...
  Крупно — только текст хадиса (matn); иснад и источник — тихими строками
  по-русски. Верный ответ листается сам (черта под «Верно»), после ошибки —
  кнопка «Дальше».
...
DAILY_TARGET = 5
# Одно новое слово в день - правило методики (пользователь 23.09: «ещё одно
# слово» его ломает). Повторять выученное можно сколько угодно - «Ещё заход».
NEW_PER_DAY_MAX = 1
...
def full_text(n, i):
    ...
    h = data()[n]
    P = h["phrases"]
    # Только сам текст хадиса (24.09.2026, решение пользователя): цепочку
    # передатчиков и «передал аль-Бухари [رقم:25]» тренажёр не спрашивает,
    # крупно они только отвлекают. Текст - от matn_from до последней фразы,
    # где есть учимые слова; до него - иснад, после - источник. Оба идут
    # тихой строкой по-русски.
    phs = [w["ph"] for w in h["words"] if w.get("ph") is not None]
    lo, hi = h.get("matn_from", 0), max(phs) if phs else len(P) - 1
    cur = _phrase_of(n, i)
    matn = [{"ar": p["ar"], "ru": p["ru"]} for p in P[lo:hi + 1]]
    # Кавычки прямой речи по краям текста без иснада висят сиротами.
    for k in ("ar", "ru"):
        matn[0][k] = matn[0][k].lstrip(_EDGE)
        matn[-1][k] = matn[-1][k].rstrip(_EDGE)
    return {"n": n,
            "isnad": _tidy(" ".join(p["ru"] for p in P[:lo])),
            "phrases": matn,
            "source": _tidy(" ".join(p["ru"] for p in P[hi + 1:])).strip("() ."),
            "cur": cur - lo if cur is not None and lo <= cur <= hi else None}
...
def _tidy(s):
    return re.sub(r"\s+([,.:;!?])", r"", re.sub(r"\s+", " ", s or "")).strip()
