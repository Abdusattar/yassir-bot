...
@with_auth
async def handle_ustaz_reviewed(request, user_id):
    """GET ?group= - что уже разобрано в этой группе, свежее сверху."""
    if not _is_ustaz(user_id):
        return web.json_response({"error": "not_ustaz"}, status=403)
    try:
    ...
    return web.json_response({
        "items": get_reviewed_submissions([group_id]),
        "today": get_date(),
    })


@with_auth
async def handle_my_month(request, user_id):
    """GET - свой месяц по дням для календаря в "Сдачах". Тот же расчёт, что
    видит устаз в зоне "Студенты": у обоих должна быть одна картина, иначе
    разговор "ты пропускал" снова превращается в спор."""
    user = find_user_by_phone(user_id)
...
_login_hits = {}
# Пороги нарочно щедрые. Один адрес - это НЕ один человек: сотовый оператор
# держит сотни абонентов за общим адресом (CGNAT), дома за одним Wi-Fi сидит
# семья, в мечети - все сразу. Первый порог стоял на 20 и запер живого
# человека в первый же день (10.09.2026, телефон пользователя оказался в той
# же сети, что и компьютер, с которого шла проверка).
#
# Тесниться тут незачем: подобрать код входа нельзя (128 бит на 15 минут), и
# единственное, что этот счётчик реально сторожит, - чтобы сорвавшийся клиент
# не набивал таблицу строками. С этим справляется и щедрый потолок.
_LOGIN_MAX_PER_HOUR = 120         # на один внешний адрес
_LOGIN_MAX_PER_HOUR_SHARED = 600  # когда адрес неизвестен и все слиты в один
