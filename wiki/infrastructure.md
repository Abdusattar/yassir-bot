...
**СНЯТО 23.09.2026: деплой перезапускает оба бота сам.** В
`.github/workflows/deploy.yml` после `systemctl restart yassir-bot` идёт
`systemctl --user restart yassir-bot-female.service` (без sudo — деплой
ходит на сервер пользователем `stursunkul`, а его sudoers пускает только
мужской юнит), и затем проверка: оба процесса должны ответить на своих
портах `localhost:8081` и `8082`. Порт открывается только после успешного
getMe и инициализации базы, поэтому проверка ловит и «поднялся и упал».
Не ответили — деплой красный. Ручной рестарт женского больше не нужен.

Почему это сделали: два процесса стоили нам не удвоенной сложности, а
«полутора деплоев» — и однажды это вышло неделей старого кода (ниже).
Заодно ушла невольная «канарейка»: раньше плохой коммит сначала ломал
только мужскую половину, теперь вместо неё — проверка после рестарта.

**Как было до 23.09.2026**: после пуша в мужской бот код женского на диске
