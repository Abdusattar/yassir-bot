...
общего потолка, иначе «20 попыток в час» заперли бы всех студентов разом.
Но различать всё-таки лучше.

Проверять поведением, а не конфигом: 21-й подряд `POST /api/muf/male/auth/start`
с одного адреса обязан ответить 429. Если заголовок не доходит, все сливаются
в одного и работает потолок 300 — 429 не наступит.
