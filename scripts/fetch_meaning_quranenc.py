...
Права: разрешение на размещение этих переводов в нашем проекте
подтверждено пользователем 30.08.2026 ("разрешения есть на наш проект") -
тот же порядок, что с русским переводом Кулиева (см.
scripts/add_ayah_meaning.py) и со шрифтом/вёрсткой QUL: точность источника
и право на переразмещение подтверждаются отдельно, и то и другое - через
человека, не по догадке скрипта.
